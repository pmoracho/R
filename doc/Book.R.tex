...
pero a veces es "invisible" y no se imprime vlor alguno en la consola.
\newpage
\begin{figure}[!htb]
	\centering
...
exclusivamente vectores numéricos, pero en este capítulo se introduciremos varios
...
\textbf{R} se hacen usando llamadas de funciones, comandos que parecen una
aplicación de una función matemática de una o varias variables; por ejemplo,
\texttt{log (x)} o \texttt{plot(height, weight)}. El formato es que a un nombre
de función le sigue un conjunto de paréntesis que contiene uno o más
...
\texttt{weight}.  Estos son los argumentos reales, que sólo se aplican a la
convocatoria actual. Una función también tiene argumentos formales, que se
conectan con argumentos reales en la llamada.

...
función \texttt{ls} (lista).  Si ha ejecutado todos los
ejemplos del capítulo anterior es posible que la salida sea similar a esto:
...
acceder luego por medio del menu ``Ayuda''.  Se puede acceder también a una
...
descargados from \textbf{CRAN} (ver apéndice A), el cual alberga a la fecha más
de 12000 paquetes para múltiples propósitos.  Incluso podríamos crear nuestros
propios paquetes. Una librería es por lo general una simple carpeta en el
disco. Una librería de sistema se crea al instalar \texttt{R}.  En algunas
instalaciones, no les es permitido a los usuarios modificar la libreía del
...
usuarios probablemnet no necesiten tener cargada todo el tiempo. Un paquete es
...
llegaramos a necesitar hacer, podríamos escribir:
...
With this mechanism, data are “just there”. For example, if you type “thue-
sen”, the data frame of that name is displayed. Some packages still require
...
What data does is to go through the data directories associated with each
...
If there is a subdirectory of the current directory called data, then it
is searched as well. This can be quite a handy way of organizing your
personal projects.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{\texttt{attach} and \texttt{detach}}\label{attachdetach}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

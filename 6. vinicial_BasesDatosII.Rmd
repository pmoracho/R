...
Esto `breaks= c(-43,-1,15, 1301)` tiene un problema, que lo podemos apreciar con este ejemplo

```{r}
cut(c(-43, -1, 0, 13, 15, 18),  
    breaks= c(-43,-1,15, 1301), 
    labels= c("Antes de tiempo", "A tiempo", "Retrasado"))
```

Si bien, el rango mínimo es -43, el `break` no es inclusivo, debiéramos indicar en todo caso `-44` pero mucho mejor, es definir los extremos de esta forma:

```{r}
cut(c(-43, -1, 0, 13, 15, 18),  
    breaks= c(-Inf,-1,15, Inf), 
    labels= c("Antes de tiempo", "A tiempo", "Retrasado"))
```

Usando `Inf` y `-Inf` terminamos de cubrir todo el espectro de los números naturales en este caso, pero funciona con los reales también. Definir los cortes de esta forma es "a prueba de balas", cualquier nuevo valor más allá de los extremos estaría contemplado. El usar números concretos para los extremos, puede tener utilidad, cuando solo nos interesa ciertos rangos y lo que salen de ellos, directamente no los queremos contemplar y los dejamos como `NA`.
...
Esto `factor(vuelos$month, labels=meses, ordered=TRUE)` tiene un potencial y yo diría bastante habitual problema, veamos este caso:

```{r echo=TRUE, message=TRUE, warning=TRUE, error=TRUE}
meses <- c("Ene","Feb","Mar","Abr","May","Jun", "Jul", "Ago", 
	"Sep", "Oct","Nov","Dic")
factor(c(1,2,12), labels=meses, ordered=TRUE)
```
Si en nuestra variable, no tenemos todos los valores esperados, no podremos convertirla a factor. En el ejemplo solo tenemos 3 de los posibles 12 meses. Para solucionar esto, es mucho mejor ser más explícitos:

```{r}
meses <- c("Ene","Feb","Mar","Abr","May","Jun", "Jul", "Ago", 
	"Sep", "Oct","Nov","Dic")
factor(c(1,2,12), levels=1:12, labels=meses, ordered=TRUE)
```

Con `levels` definimos los valores esperados en nuestra variable, no importará si no existen, el `factor` guardará ese lugar
...
Entiendo el ejemplo que quieres dar, pero, cuando se trata de pasar a un factor una columna numérica, puedes hacerlos directo sin pasar por el `ifelse()`

```{r}
set.seed(12345)
df <- data.frame(sexo=sample(c(1,2),15,replace=TRUE))
df$hm <- factor(df$sexo, labels = c("Mujer","Hombre")); df
```

Simplemente hay que dar en orden las etiquetas que quieres para cada valor.
...
Esto es una nota de estilo, puedes obviarla. En mi caso el código anterior lo escribiría así

```{r eval=FALSE}
niveles_educ %>% 
  mutate(grupo_tidy = case_when( 
      niveles%in%c("Preescolar", "Primaria", "Secundaria",
        "Secundaria técnica") ~ "Básica",
      niveles%in%c("Bachillerato general", 
        "Bachillerato técnico") ~ "Media superior",
...
* Primero el `data.frame`
* luego el verbo (sin el namespace `dplyr::`) en algunos caso cuando hay conflicto con otro paquete si lo uso obviamente
* finalmente la asignación a derecha -> con un nuevo nombre de objeto. Prefiero que cada lote de comandos `tidy` generen un nuevo objeto, por que muchas veces tengo que volver a atrás y me resulta útil seguir conservando los datos originales del lote, para volver a correr el lote de comandos.
...
Ya que se trata de un curso de datos, te sugiero una forma de resolver este problema, que en algunos casos es mucho más clara. Se trata de usar una tabla de equivalencias, dónde cada `nivel` lo asociamos a un `grupo_baser`. algo como esto:
...
niveles_educ <- data.frame(niveles=c(sample(niveles, 50, replace=TRUE)), stringsAsFactors = FALSE)

# 
# Ahora definimos nuestras equivalencias, da nivel mapea un grupo_baser
# 
tribble(
  ~niveles,                         ~grupo_baser,
  "Preescolar",                     "Básica",
  ...
  "Secundaria",                     "Básica",
  "Secundaria técnica",             "Básica",
  "Bachillerato general",           "Media superior",
  "Bachillerato técnico",           "Media superior",
  "Pedagógica",                     "Superior",
...
la relación finalmente la resolvemos con un `left_join` aunque bien podría hacerse mediante un `match()` de R base. Cuando las transformaciones son muchas, este mecanismo es más cómodo y organizado.
...
Una forma más simple, que te evita el `ifelse()` y el `paste0()` es formatear la cadena mediante `sprintf()`, habría que dar una introducción a los patrones de formateo de esta función, al menos los más simples. Te permitiría resolver todo así:

```{r eval=FALSE}
horas_min = sprintf("%02d:%02d", horas, min)
```
***
...
La sintaxis `ecdf(resultado)(resultado)` podría llamar la atención de alguno. Se explica si menciones que `ecd()` en vez de retornar un dato, retorna una función:

```{r eval}
class(ecdf(test$resultado))
```

Por lo que finalmente `ecdf(resultado)(resultado)` terminará siendo algo asi:
`funcion_retornada_por_ecdf(resultado)` 
...
Es útil mencionar, en el caso de `mutate_at()` y de otros verbos de `dplyr` el uso `vars()` así:

```{r eval=FALSE}
select(vuelos, c(carrier, distance,vel_kmh)) %>%
  mutate_at(vars(distance, vel_kmh), log)
```

Usando `vars()` escribes las variables de la forma habitual en `dplyr`, sin comillas, pero aparte te permite usar los "helpers" de selección `?select_helpers` 
...
`tidyverse` y `ggplot` explotan mucho el concepto del `tidy data`, la idea: _"Cada fila es una observación, cada variable  es una columna"_, si se va a trabajar con estos paquetes va a ser necesario remodelar los datos a un formato `tidy`. No se si lo has visto, pero es muy interesante la lectura del capitulo de ["Datos ordenados"](https://es.r4ds.hadley.nz/datos-ordenados.html) del libro de .
...
Bien tu comentario acerca de la normalización de los nombres de variables. Me detendría un poco más explicando que es un nombre de variable adecuado y cual no y por qué. Por ejemplo, la molestia que significa trabajar con nombres con espacio o caracteres especiales, o nombres demasiado largos. 

¿Cómo es un nombre sintácticamente correcto en R?

* Combinan letras, dígitos, punto (.) y guión bajo (_).
* Comienzan con una letra o un punto, si comienzan con un punto, no puede continuar por un dígito.
* Las palabras reservadas en R no pueden utilizarse.

Acerca del uso del `.` hay opiniones que sugieren no usarlo ya que es una notación muy usada en las funciones genéricas de OOP.

Es útil usar `make.names()` para hacer válido algo que no lo es:

```{r}
make.names("Este es un nombre invalido")
```

Aparte de `View()` te recomiendo `dplyr::glimpse()`  que es una especie de `str()` mejorado.

Por otro lado, no tengo nada para comentarte acerca de las funciones `melt()` o `dcast()`, ya que he preferido siempre ir por el lado `tidyverse`, lo único que podría decirte, pero es una cuestión de criterio totalmente opinable, es si vale la pena abrir el juego a dos herramientas en un curso introductorio, ¿sirve? ¿confunde? es una pregunta no una aseveración. 
...
**`dcast()`** y **`pivot_wider()`** incrementan el número de columnas y disminuyen el de renglones. Esto es útil si requerimos presentar información o un índice consecutivo (variable **X**). Supongamos que queremos saber cuál es el promedio que otorgan hombres y mujeres según su ocupación para la variable **rank**. Con **`dcast()`** podemos lograr esto de forma muy sencilla:
...
Si bien, es correcto que el principio de combinación mediante `cbind` o `bind_cols` sea el tener la misma cantidad de filas, realmente, el principio fundamental es que las filas de un `data.frame`, tengan una relación con el otro, que en el caso de estas funciones esta limitada técniamente a una relación 1 a 1 y a un orden en los datos que haga coincidir de manera coherente una fila cono otra. Conceptualmente, estas funciones, entran igual dentro de los principios de bases de datos relacionales, solo que la clave relacional, en este caso, es el número de fila. 
...
Alejandro, en mi opinión, yo no me metería con los conceptos de claves primarias o foráneas, son conceptos técnicos de las bases de datos, que los has planteado correctamente pero no son conceptos útiles para el trabajo con los datos en R, trataré de explicarte:

* En BR (Bases relacionales) hay tres aspectos bien separados
  * Diseño
  * Administración
  * Consulta
  
* Un usuario típico de R solo alcanza a ver el último punto. Si bien es cierto que entender algo del diseño de BR es útil, siempre va a ser a un nivel más macro.
* Los conceptos de claves primarias y/o foráneas, son conceptos que se ven en diseño y administración, pero es algo invisible en las consultas

**¿Por que los conceptos de PK o FK no aportarían nada?**

* Una tabla puede no tener PK (algunos motores lo permiten, otros no)
* Una tabla puede tener PK y aún así nunca usarse para hacer una relación 
* Las FK son para mantener la integridad referencial: no eliminar una fila si tiene una referencia en otra tabla. Un usuario de R a menos que tenga que diseñar una base de datos nunca va a necesitar entender esto y aún así la integridad referencial se puede manejar por afuera de la BD.
* Ambos conceptos dependen del concepto de índices, básicamente, estructuras externas a la tabla que permiten eventualmente mejorar la performance en múltiples consultas, pero son totalmente invisibles al usuario.

**¿Y que mencionar de BR a un futuro usuario de R?** Yo diría los conceptos básicos que le servirán para entender las relaciones y hacer luego las consultas. Algunas ideas o conceptos que podrías usar:

* El concepto fundamental que guía a las BR es la no repetición de los datos (es una super simplificación, por que hay otros conceptos importantísimos, pero no quiero entrar en la teoría)

* Las BR son Bases de datos que particionan la información en múltiples tablas, y permiten "relacionar" distintas tablas por medio de una o más columna de las mismas. 

* Un ejemplo simple, si tengo una base de datos de presupuestos que le he hecho a los clientes ¿Es necesario que cada uno de los presupuestos tenga todos los datos del cliente (nombre, dirección, tel y un largo etc)? ¿Qué pasa cuando un cliente cambia su nombre, teléfono, etc? ¿Hay que trasladar este cambio a todos los presupuestos? Aquí entra el concepto de bases relacionales, cada presupuesto, no tiene los datos del cliente, solo tiene uno, el código, número o id de cliente (como lo quieras llamar, por convención le decimos ID) este dato/columna/variable, nos dice cual es el cliente del presupuesto y nos da la relación directa 1 a 1 con la tabla de los Clientes (1 presupuesto 1 cliente), que por supuesto también tiene su ID de cliente, junto con todos los datos propios del mismo.

* Este es un concepto distinto a como se suelen manejar los datos en estadísticas, dónde se suele trabajar con una única tabla (que llaman base de datos) con todos los datos. Este enfoque seguramente no cambie, aún partiendo de una BR, seguramente necesitemos llegar a una estructura mas plana para realizar un análisis estadístico clásico, por eso es importante saber hacer las "consultas" que nos permitan hacernos de todo los datos que necesitemos.

* Las bases relacionales trabajan con datos "vivos" que cambian todo el tiempo, se agregan, se quitan, cambian, en estadísticas trabajamos con "fotos" de los datos, una instantánea en un determinado momento, pero aún así, para hacer esa foto, necesitaremos entender como funcionan las Bases relacionales.

* Las relaciones entre tablas, pueden ser 1 a 1 como ya dijimos en el caso de 1 presupuesto tiene 1 cliente, o bien pueden ser 1 a n, por ejemplo si viéramos la relación en sentido inverso 1 cliente pude tener muchos presupuestos.

* La relación entre tablas puede darse de forma natural y coherente: presupuestos.id_cliente = clientes.id_cliente, sin embargo, el concepto relacional es agnóstico de los datos. Las relaciones entre los datos, es lo que tu digas que son, en el momento en que haces una consulta, incluso no necesariamente tengan que ser relaciones coherentes, mientras los tipos de datos que uses para relacionar sean los mismos, ningún motor de BD te va a decir nada, el resultado serán datos más o menos coherentes. 

* La idea por supuesto no es hacer relaciones incoherentes, el punto anterior es para remarcar la idea que las relaciones no son algo rígido, sino algo dinámico y existen más allá de la implementación técnica (que podrían ser los conceptos de PK o FK)

* En bases de datos, la referencia obligada es el SQL (structured query language). Te recomiendo ver alguna referencia mínima, `dplyr` esta fuertemente influenciado por este lenguaje. Todo lo que un usuario de R puede necesitar saber en cuanto a las Bases relacionales se pueden resolver leyendo este estándar o mucho mejor alguna introducción o tutorial. 
...
En este ejemplo yo diría algo un poco más abstracto, `airlines` es una tabla cuya columna "clave" es `carrier` y  en `flights` cada fila que representa un vuelo, por supuesto pertenece a una `airlines` y tiene a su vez, la columna `carrier` (si querés la podes llamar clave secundaria). Le decimos claves o llaves primarias a aquellas que identifican a la entidad/tabla, y secundaria, cuando están en otra tabla. No siempre las relaciones son entre primarias y secundarias, muchas veces pueden ser entre secundarias y secundarias (de la misma entidad).
...
Me hace un poco de "ruido" que hables de unión, cuando efectivamente existe el concepto de `union` en SQL y es distinto a este, pero la traducción más natural de `join` es esa. Lo que sí, tal vez resaltaría aquí, la naturaleza "horizontal" de este tipo de unión.
...
Tener en cuenta que las columnas que tienen el mismo nombre en cada tabla, pero no forman parte de la clave de relación entre ambas, se renombran automáticamente con el postfijo `.x` y `.y`, esto habría que hacerlo notar, por que es habitual que cuando ocurra esto, no encontremos las columna que teníamos antes de la relación.

```{r}
df1 <- data_frame(id=1, columna="A")
df2 <- data_frame(id=1, columna="B")
...
`tidyverse` hace un muy buen trabajo para encontrar las columnas en común entre cada tabla, pero es mejor que indiquemos explícitamente las columnas claves con el `by`.
